...
RSpec.describe Song, type: :model do
  ...
  describe "class methods" do
    ...
    it "Get the titles of the songs with a play count greater than zero, sorted alphabetically" do
      query = Song.where("play_count > 0").order(title: :asc).pluck(:title)
    ...
    it "Get the length of the song with the most plays" do
      query = Song.order(play_count: :desc).limit(1).pluck(:length).first #add .first because pluck returns an array
